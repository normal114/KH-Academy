...
public class CalController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CalDao dao = new CalDao(); // 여기다 한 번에 만들자.
		
		try {

			if (command.equals("calendar")) {
				response.sendRedirect("calendar.jsp");
			} else if (command.equals("insert")) { // insert.jsp에서 보낸 argument(name)들을 받아온다.
			...
			request.setAttribute("msg", "command 오류"); // 오류 페이지로 이동
			dispatch("error.jsp", request, response);
		}
	}
	...
	private void dispatch(String path, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		RequestDispatcher dispatch = request.getRequestDispatcher(path); // error 페이지 만들기
		dispatch.forward(request, response); // throws 시키기
	}
